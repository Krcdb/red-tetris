...
import { type Board, initialBoard, type Piece } from "../utils/tetris";
...
interface GameState {
	...
	linesCleared: number;
	level: number;
	status: "idle" | "setup" | "playing" | "paused" | "gameOver";
	...
	reducers: {
		...
		updateGameState: (
			...
			if (currentPlayer) {
				...
				if (
					currentPlayer.nextPieces &&
					Array.isArray(currentPlayer.nextPieces)
				) {
					state.nextPieces = currentPlayer.nextPieces;
					console.log(
						"Setting nextPieces from current player:",
						currentPlayer.nextPieces,
					);
				}
